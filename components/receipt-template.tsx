...
export const ReceiptTemplate = React.forwardRef<HTMLDivElement, ReceiptTemplateProps>(
  ({ order, orderItems, businessInfo }, ref) => {
    ...
    const change = totalPaid > order.total_amount ? totalPaid - order.total_amount : 0
    const paymentMethod = order.payment_method || "N/A"

    // QR Code data
    const qrData = `Order: ${order.order_number} | Customer: ${order.customer?.name || "N/A"} | Date: ${new Date(order.created_at).toLocaleDateString("id-ID")} | Status: ${order.order_status}`

    return (
      <div ref={ref} className="p-4 bg-white text-black text-xs font-mono w-[300px] mx-auto">
        ...
        <div className="border-t border-b border-dashed py-2 mb-2">
          ...
          <div className="flex justify-between">
            <span>Selesai</span>
            <span>{order.delivery_date ? formatDateTime(order.delivery_date) : "Belum selesai"}</span>
          </div>
        </div>
        ...
        <div className="border-t border-b border-dashed py-2 mb-2">
          {orderItems.map((item) => (
            <div key={item.id} className="mb-2">
              ...
              <p>
                {item.quantity} {item.service?.unit || "kg"} x {formatCurrency(item.unit_price)}
              </p>
              <p className="text-right">Subtotal: {formatCurrency(item.subtotal)}</p>
              {item.notes && <p className="text-xs italic">Catatan: {item.notes}</p>}
            </div>
          ))}
        </div>
        ...
        <div className="border-t border-b border-dashed py-2 mb-4 text-center">
          <p>Dengan mencuci di {businessInfo.name}, maka Customer telah setuju dengan ketentuan yang berlaku:</p>
          <ul className="list-disc list-inside text-left mt-2">
            <li>Pakaian luntur, mudah susut & kerut karena kondisi bahan di luar tanggung jawab kami</li>
            <li>Komplain dgn nota asli & max 3x24 jam</li>
            <li>Ganti rugi: Total biaya/jumlah cucian x 10</li>
            <li>Ganti rugi cuci satuan sesuai harga barang & maksimal: harga biaya cuci x 10</li>
            <li>Kami tidak bertanggung jawab utk cucian yg tdk diambil dlm waktu 14 hari</li>
          </ul>
