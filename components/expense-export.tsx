...
export function ExpenseExport({ expenses }: ExpenseExportProps) {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      if (dateRange.startDate || dateRange.endDate) {
        const dateRangeText = `Periode: ${dateRange.startDate || "Awal"} - ${dateRange.endDate || "Akhir"}`
      ...
      // Set column headers
      const headerRow = worksheet.addRow(["Tanggal (otomatis harian)", "Jenis Pengeluaran", "Rp"])
      ...
      // Add note
      worksheet.addRow([])
      const noteRow = worksheet.addRow(["", "Dibuat drop down list", ""])
