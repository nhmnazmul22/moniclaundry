...
export async function GET(request: NextRequest) {
  try {
    ...
    const depositTypes = await DepositTypeModel.find(matchStage).sort({ createdAt: -1 })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, purchase_price, deposit_value, branch_id, description } = body

    if (!name || !purchase_price || !deposit_value || !branch_id) {
      return NextResponse.json(
        {
          status: "Failed",
          message: "Name, purchase price, deposit value, and branch ID are required",
        },
        { status: 400 },
      )
    }
    ...
    const depositType = await DepositTypeModel.create(body)
